...
def model_process(dataset = dataset):
    ...
    # Fixed predictors and response assignment
    predictors = pred
    response = fig
...
def graph_process(dataset):
    ...
    if histogram:
        figs['Histograms'] = plotfun.plot_histsmooth(ds, histogram)
    if corr:
        figs['Correlations'] = plotfun.plot_correlations(ds, corr)
    if boxplotcat and boxplotnum:
        figs['Box Plot'] = plotfun.plot_boxplot(ds, boxplotcat, boxplotnum)  
